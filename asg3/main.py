...
def load_data_sets(N_training=1000):
  ...
  N_cutoff = int(floor(float(N_training)/2.0))

  if N_cutoff >= len(nonspam) or N_cutoff >= len(spam):
  ...
  shuffle(nonspam)
  shuffle(spam)
  nsp_training = nonspam[:N_cutoff]
  sp_training = spam[:N_cutoff]
  nsp_testing = nonspam[N_cutoff:]
  sp_testing = spam[N_cutoff:]
...
if __name__ == '__main__':
  ...
  cli.add_argument('-n', '--ntraining', default=1000, type=int,
                   help='The number of emails to allocate for training data. Default: 1000, Max: 1200')
  args = cli.parse_args()
